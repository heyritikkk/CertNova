...
import { getQuizForModule, evaluateStudentPerformance } from '../lib/moduleQuizzes';
import './ModuleExamPortal.css';

const EXAM_DURATION = 1800; // 30 minutes in seconds

export default function ModuleExamPortal({ moduleId, moduleTitle, courseId, onComplete, isCompleted }) {
  const questions = useMemo(() => getQuizForModule(moduleId, moduleTitle), [moduleId, moduleTitle]);
  ...
  const handleFinishExam = (autoSubmitted = false) => {
    ...
    if (passed && onComplete) {
      onComplete(`module-quiz-${moduleId}`);
    }
  ...
  const performanceReport = useMemo(() => {
    ...
    return evaluateStudentPerformance(questions, answers);
    ...
    return (
      <div className="exam-portal exam-portal--intro">
        <div className="exam-intro-badge">
          ...
          <span>CompTIA Security+ Practice Assessment</span>
        </div>
        
        <h2>{moduleTitle} Practice Exam</h2>
        <p className="exam-intro-desc">
          Test your comprehension of the concepts covered in this module. This practice exam simulates standard CompTIA test parameters to prepare you for the actual certification test environment.
        </p>
        ...
        <div className="exam-rules-list">
          ...
          <ul>
            ...
            <li>Upon completion, you will receive a detailed performance audit detailing your Strengths and Areas of Improvement (Pros and Cons).</li>
          </ul>
        </div>
      ...
      <div className="exam-portal exam-portal--active">
        <div className="exam-active-header">
          <div className="exam-back-header">
            <span className="exam-back-arrow">←</span> {moduleTitle}
          </div>
        </div>
