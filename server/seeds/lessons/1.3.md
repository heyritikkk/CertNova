...
* **Physical Layer (L1):** The physical envelope and mail trucks. An attacker can cut open the mailbox or steal the truck.
  > *Real life example:* Think of sending a letter through the postal service. The physical envelope and mail trucks. An attacker can cut open the mailbox or steal the truck.
* **Data Link Layer (L2):** The sorting facility within your local neighborhood. An attacker can trick the sorter into putting your mail into a neighbor's box (ARP Poisoning).
  > *Real life example:* The sorting facility within your local neighborhood. An attacker can trick the sorter into putting your mail into a neighbor's box (ARP Poisoning).
* **Network Layer (L3):** The global addressing system (ZIP codes and street addresses). An attacker can put a fake return address on the envelope (IP Spoofing).
  > *Real life example:* The global addressing system (ZIP codes and street addresses). An attacker can put a fake return address on the envelope (IP Spoofing).
* **Transport Layer (L4):** The delivery option - Certified Mail requiring a signature confirmation (TCP 3-way handshake) vs. Standard Stamps thrown in a box (UDP). An attacker can flood the recipient's mailbox with fake certified mail slips until they cannot receive real mail (SYN Flood).
  > *Real life example:* The delivery option - Certified Mail requiring a signature confirmation (TCP 3-way handshake) vs. Standard Stamps thrown in a box (UDP). An attacker can flood the recipient's mailbox with fake certified mail slips until they cannot receive real mail (SYN Flood).
...
* **The Attack (Layer 2 ARP Poisoning):** An attacker on a local LAN runs `arpspoof -i eth0 -t 192.168.1.5 192.168.1.1`. This sends forged ARP responses to the victim (192.168.1.5), claiming that the attacker's MAC address belongs to the default gateway (192.168.1.1). The victim’s traffic now routes directly through the attacker before reaching the internet.
  > *Real life example:* An attacker on a local LAN runs `arpspoof -i eth0 -t 192.168.1.5 192.168.1.1`. This sends forged ARP responses to the victim (192.168.1.5), claiming that the attacker's MAC address belongs to the default gateway (192.168.1.1). The victim’s traffic now routes directly through the attacker before reaching the internet.
...
```plantuml
@startuml
title ARP Poisoning Attack Flow — Layer 2
...
== Normal Traffic ==
V -> SW : ARP Request: "Who has 192.168.1.1?"
SW -> GW : Broadcast
GW --> SW : ARP Reply: "192.168.1.1 is at AA:BB:CC:DD"
SW --> V : Forward reply — ARP cache updated

== ARP Poisoning Attack ==
ATK -> SW : Forged ARP Reply:\n"192.168.1.1 is at ATTACKER:MAC"
SW -> V : Forwarded (no DAI yet)
note over V : Victim ARP cache\npoisoned!

V -> SW : Packet to 192.168.1.1 (gateway)
SW -> ATK : Traffic redirected to attacker
ATK -> GW : Forwards packet (Man-in-the-Middle)

== Defense: Dynamic ARP Inspection Enabled ==
ATK -> SW : Forged ARP Reply (again)
SW -> SW : Check DHCP Snooping binding table
note over SW : IP-to-MAC mismatch\ndetected!
SW --> ATK : Packet DROPPED at hardware port
@enduml
```
